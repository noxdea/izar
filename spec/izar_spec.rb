...
RSpec.describe Izar do
  def repository
    ...
    system("git", "-C", dir, "config", "core.autocrlf", "false")
    system("git", "-C", dir, "config", "maintenance.auto", "false")
    File.binwrite(File.join(dir, "README.md"), "one\ntwo\n")
  ...
  it "rejects empty commits and protects discard behind confirmation" do
    ...
    File.binwrite(File.join(dir, "README.md"), "changed\n")
  ...
  it "filters through the interactive session and exposes messages to the view" do
    repo, dir = repository
    File.binwrite(File.join(dir, "notes.txt"), "notes\n")
    model = Izar::Model.new(dir)
  ...
  it "renders a large highlighted diff through the virtualized view" do
    repo, dir = repository
    File.binwrite(File.join(dir, "README.md"), (1..1_000).map { |line| "line #{line}\n" }.join)
    model = Izar::Model.new(dir)
